...
def main():
    ...
    with tf.Session() as sess:
        ...
        predictions = graph.get_tensor_by_name("q_estimator/linear/predictions:0")
        X = graph.get_tensor_by_name("q_estimator/X:0")
        real_seq_length = graph.get_tensor_by_name("q_estimator/real_seq_length:0")

        epsilon = 0.5
        while True:
            feed_dict = {X: np.expand_dims(state, 0), real_seq_length: np.expand_dims(seq_len, 0)}
            q_values = sess.run(predictions, feed_dict)[0]
            action_probs = np.ones(actionProcessor.actionNum, dtype=float) * epsilon / actionProcessor.actionNum
